...
func perform_target(_skill: SkillModel, _source: UnitModel):
...
func perform_target_mode(_skill: SkillModel, _source: UnitModel):
  if Input.is_action_just_pressed("ui_left"):
    set_current_target(current_target - 1)
    perform_select_target(_skill, _source)
